...
// exist at all. A slot glyph is `slot_h * 85/100` — 15 px on flint and 23 on both the
// others — so the divisor of nine this used to carry landed stroke_px() on 1 for every
// one of them, on every display. Measured on emery, which is not even the small one: a
// sharp-left maneuver whose shaft is a single pixel under a filled head reads as a blob
// with a whisker, and the phone silhouette reads as an empty box.
//
// Three, because stroke_px() yields only odd widths and there is nothing between a
// hairline and three. The floor was at 16 px of glyph while the slot font was a 16 px
// TTF; Gothic 14's box is two pixels shorter, which took flint's glyph to 15 and
// dropped it back through the floor to a hairline — the exact failure this constant
// exists to prevent, arriving by way of a font change rather than a divisor.
//
// Fourteen now, and the two pixels were measured rather than assumed: at 15 px of glyph
// with a 3 px stroke flint's phone silhouette keeps four pixels of interior and still
// reads as an outline. Below that it would not, which is why there is a floor on the
// floor at all — the battery and the calendar never reach it, being mostly interior and
...
// against the strip. See ROW_ALIGN. The glyph leads: it is what says which kind of row
// this is, so it belongs on the edge the eye arrives at.
//
