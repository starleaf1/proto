...
// ---------------------------------------------------------------------------
// The strip is a timeline down the left edge, later always lower: one hour above the
// pointer and three below it on the rectangles, one and a half and three and a half on
// gabbro, whose strip runs off the glass at both ends — see STRIP_BACK_S.
//
...
// small and this is 241 bytes (301 on gabbro) that would
...
//
// Sized from the window. The walk includes both ends, so an n-hour window holds n+1 of
// these when `now` sits exactly on the hour — five — and one spare past that. A literal
// here is how a six-hour window once overran it by one; the bound check below means
// that cost the last hour label silently rather than crashing, which is the worse of
// the two.
...
//
// `label_x` is a depth along the ray, the same way the markers' depths are, so the lane
// is simply a vertical column beside the strip.
static void format_hour(char *buf, size_t n, int hh) {
  if (clock_is_24h_style()) {
    snprintf(buf, n, "%02d", hh);
  } else {
...
static void draw_hour_label(GContext *ctx, const Layout *lo, GFont font,
  ...
  //
  // The label is small enough that integer division is a design decision: an eighth —
  // what the Rajdhani resource this replaced wanted — truncates to one here and left the
  // number a pixel below the notch it names.
  int16_t lift = ts.h / 6;
  GRect box = GRect(q.x - lo->label_w / 2, q.y - ts.h / 2 - lift, lo->label_w, ts.h);
