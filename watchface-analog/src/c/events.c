...
// The far tier reaches exactly as far as the ring does, deliberately: the
// countdown should never name something the reader cannot also see a marker for.
#define SLOT_FAR_S   RING_AHEAD_S
...
// the ring blank for up to a quarter of an hour. Blank does not read as "waiting",
...
// slot says the companion is gone, and an empty ring looks exactly like a free
// afternoon.
//
...
// better reading than one period of an empty ring.
// ---------------------------------------------------------------------------
...
// back as nothing, which is a blank ring and no other symptom. Break the build
...
bool event_visible(const Event *e, time_t now) {
  ...
  if (e->start > now + RING_AHEAD_S) return false;        // past the horizon, for now
...
void events_gc(time_t now) {
  for (int i = 0; i < EVENTS_MAX; i++) {
    ...
    // window is not dead — the leading end cap sweeps past it in time.
