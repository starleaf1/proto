...
// without a watchface update.
//
// We allow three periods, which is the companion's own retry ladder read from this
// end. Its slow tier ticks at 600 s, and a tick whose flush does not land backs the
// next attempt off to 1200 s -- so one miss puts the next real attempt 1800 s out, and
// alerting before that would be alerting while the companion is still visibly working
// the problem. Three periods is exactly where "it missed one and the retry missed too"
// begins, and that is a dead companion rather than a throttled one.
//
// This was 2.5 periods against a 900 s tier, which had no retry to account for. Note
// that the absolute grace got *shorter*, not longer: 1800 s against the old 2250 s. The
// pair stays a ratio rather than collapsing to one number so a fractional multiplier is
// still expressible.
#define HB_GRACE_NUM 3
#define HB_GRACE_DEN 1
#define HB_MIN_S     15      // clamp a garbled period into something survivable
#define HB_MAX_S     3600
#define HB_DEFAULT_S 600     // assumed cadence before the companion declares one
